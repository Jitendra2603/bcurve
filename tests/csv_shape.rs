...
fn schedule_has_one_header_and_no_demo_cols() {
    ...
    let header_count = s.lines().filter(|l| l.starts_with("bin,price,delta_x")).count();
    assert_eq!(header_count, 1, "CSV must have exactly one header row");
    
    assert!(!s.contains("surcharge_launch_pct"), "no example surcharge columns");
    assert!(!s.contains("fee_total_plus_surcharge"), "no example surcharge columns");
    
    // Verify correct column structure
    let header_line = s.lines()
        .find(|l| l.starts_with("bin,"))
        .expect("should have header line");
    assert_eq!(header_line, "bin,price,delta_x,supply_cum,revenue_bin,revenue_cum,fee_base,fee_var,fee_total");
